...
class SledujuFilmyContentProvider(ContentProvider):
    ...
    def list_series(self, url):
        result = []
        url += '/abecedni-seznam/'
        if not self.quickparser:
            for ix in xrange(1,35):
                url = 'http://serialy.sledujufilmy.cz/abecedni-seznam/'
                data=util.substr(urllib2.urlopen(urllib2.Request('%s/?pg=%s' % (url,ix))).read(),'div class="movies_list"','div class="pagination"')
